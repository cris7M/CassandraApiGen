...
public class NewTableCreation {
	...
	public void Customer_details() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		...
		createTable1 = "CREATE TABLE IF NOT EXISTS "+Constants.CASSANDRA_KEYSPACE+"."+Constants.CASSANDRA_CUSTOMER_DETAILS_TABLE+"(meid text, dealer_name text, meid text, "
				+ "home_contact text, work_contact text, email text, address1 text, address2 text, city text, state, country text, zipcode text, primary key(meid));"; 
		try{
	...
	public void panel_details() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		...
		part_number	text
		pca_serial_number	text
		system_serial_number	text
		arming_mode	text
		manufacturer	text
		panel_properties	map<text, text>
		dealer_name	text
		city	text
		State	text
		area_code	text
		...
		panel_status	text
		carrier_name	text
		cellular_connection	text
		cellular_signal_strength	text
		carrier_details	frozen<carrier_info>
		daughter_cards	list<frozen<daughter_card_info>>
		installation_date	timestamp
		updated_time	timestamp
		PRIMARY KEY( meid, partition)	*/
	...
	public void panel_inventory() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		...
		/*meid	text
		protocol_type	text
		panel_device_type	text
		count	counter
		PRIMARY KEY (meid, protocol_type, panel_device_type)	*/

		
		createTable4 = "CREATE TABLE IF NOT EXISTS "+Constants.CASSANDRA_KEYSPACE+"."+Constants.CASSANDRA_PANEL_INVENTORY+"(meid text, "
				+ "protocol_type text, panel_device_type text, count counter, primary key(meid, protocol_type, panel_device_type));"; 
		try{
	...
	public void panels_count_by_parameter() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		logger.info("Creating a panels_count_by_parameter Table in cassandra cluster, please wait...");
		...
		param_name	text
		count	counter
		PRIMARY KEY (dealer_name,id,param_type,param_name)*/	

		
		
		createTable5 = "CREATE TABLE IF NOT EXISTS "+Constants.CASSANDRA_KEYSPACE+"."+Constants.CASSANDRA_PANELS_BY_VERSION+"(dealer_name text, "
				+ "id uuid, param_type text, param_name text, count counter, primary key (dealer_name, id, param_type, param_name);"; 
		try{
			ResultSet resultSet5 = session.execute(createTable5);
		}catch(Exception e){
			logger.info("panels_count_by_parameter table created::: ", e);
			e.printStackTrace();
		}finally{
			//session.close();
		}
		System.out.println("panels_count_by_parameter table created");
	}
	...
	zone_id	int
	vendor_name	text
	device_properties	map<text, text>
	dealer_name	text
	...
	updated_time	timestamp
	installation_date	timestamp
	PRIMARY KEY (meid, protocol_type, zone_id, end_point_id)	
*/
	...
	public void sensor_table() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		...
		createTable6 = "CREATE TABLE IF NOT EXISTS "+Constants.CASSANDRA_KEYSPACE+"."+Constants.CASSANDRA_SENSOR_TABLE+"(meid text, end_point_id int, "
				+ "protocol_type text, zone_id int, vendor_name text, device_properties map<text, text>, "
				+ "dealer_name text, sequence_number text, sensor_type text, battery_status text, chime_type text, sensor_name text, "
		...
		try{
			ResultSet resultSet8 = session.execute(createTable6);
		}catch(Exception e){
	...
	public void panel_audit() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		logger.info("Creating a panel_audit Table in cassandra cluster, please wait...");
		String createTable9 = null;
		
		/*meid	text
		session_id	TimeUUID
		up_time	text
		panel_startup_properties	map<text, text>
		session_status	text
		session_termination_reason	text
		start_time	timestamp
		end_time	timestamp
		panel_session_stop_properties	map<text, text>
		session_type	text
		PRIMARY KEY (meid, session_id)	*/
		...
		createTable9 = "CREATE TABLE IF NOT EXISTS "+Constants.CASSANDRA_KEYSPACE+"."+Constants.CASSANDRA_PANEL_SESSION+"(meid text, "
				...
				+ "start_time timestamp, end_time timestamp, panel_session_stop_properties map<text, text>, session_type text, PRIMARY KEY (meid, session_id))"; 
		try{
			ResultSet resultSet9 = session.execute(createTable9);
		}catch(Exception e){
			logger.info("panel_audit table created::: ", e);
			e.printStackTrace();
		}finally{
			////session.close();
		}
		System.out.println("panel_audit table created");
	}
